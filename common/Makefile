...
RELEASE = 1
SPEC = rpmbuild/SPECS/puppet-common.spec
SOURCE = rpmbuild/SOURCES/puppet-common-$(VERSION).tar.bz2
SRPM = rpmbuild/SRPMS/puppet-common-$(VERSION)-$(RELEASE).src.rpm
RPM = rpmbuild/RPMS/puppet-common-$(VERSION)-$(RELEASE).rpm
SERVER = 'download.gluster.org'
REMOTE_PATH = 'purpleidea/puppet-common'

all: docs rpm

docs: puppet-common-documentation.pdf

puppet-common-documentation.pdf: DOCUMENTATION.md
	pandoc DOCUMENTATION.md -o 'puppet-common-documentation.pdf'
...
$(RPM): $(SPEC) $(SOURCE)
	...
	mv rpmbuild/RPMS/noarch/puppet-common-$(VERSION)-$(RELEASE).*.rpm $(RPM)
...
$(SRPM): $(SPEC) $(SOURCE)
	...
	#mv rpmbuild/SRPMS/puppet-common-$(VERSION)-$(RELEASE).*.src.rpm $(SRPM)

#
#	spec
#
$(SPEC): rpmbuild/ puppet-common.spec.in
	@echo Running templater...
	#cat puppet-common.spec.in > $(SPEC)
	sed -e s/__VERSION__/$(VERSION)/ -e s/__RELEASE__/$(RELEASE)/ < puppet-common.spec.in > $(SPEC)
...
$(SOURCE): rpmbuild/
	...
	git archive --prefix=puppet-common-$(VERSION)/ -o $(SOURCE) $(VERSION) 2> /dev/null || (echo 'Warning: $(VERSION) does not exist.' && git archive --prefix=puppet-common-$(VERSION)/ -o $(SOURCE) HEAD)
